...
## Returns true only if ALL required connections (except the one being used to connect) have normal rooms adjacent
## The connecting_via parameter specifies which connection is being used to connect (will be skipped in validation)
func _can_fulfill_required_connections(room: MetaRoom, position: Vector2i, connecting_via: MetaRoom.ConnectionPoint = null) -> bool:
	...
	if debug_connection_rooms and is_debug_room:
		...
		if connecting_via != null:
			...
			print("INFO: No connecting_via specified - all required connections must be presatisfied")
